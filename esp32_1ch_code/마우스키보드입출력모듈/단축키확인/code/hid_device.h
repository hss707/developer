  ...
  if ( Serial2.available() >= 6 )
  {
    ...
    if ( val == 20 )
    {
      ...
      if ( cmd == 6 ) // 키보드 신호의 경우
      {
        ...
        for ( int j = 0; j < 12; j++) // 단축키의 개수 12개
        {
          int short_key_flag = true;
          for ( int i = 0; i < 8; i++) // 키보드 신호 8바이트
          {
            if ( buf[i] != short_key[j][i] )
            {
              short_key_flag = false;
               break;
            }
          }
          if ( short_key_flag == true )
          {
            Serial.print("매크로 키 인식됨 , 매크로 번호는 ");
            Serial.print( j );
            Serial.println("");
            if ( j == 0 )
            {
